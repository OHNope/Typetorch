...
symbol_bytes() {
  ...
  local pattern="$2"
  nm -S --size-sort --demangle "$probe" 2>/dev/null |
    awk -v pattern="$pattern" '$0 ~ pattern { bytes += strtonum("0x" $2) } END { print bytes + 0 }'
}
...
tensor_bytes() {
  ...
  local namespace="$2"
  nm -S --size-sort --demangle "$probe" 2>/dev/null |
    awk -v ns="$namespace" '$0 !~ ns && /typetorchW9typetorch6Tensor/ { bytes += strtonum("0x" $2) } END { print bytes + 0 }'
}
...
tensor_unique_bytes() {
  ...
  local namespace="$2"
  nm -S --size-sort --demangle "$probe" 2>/dev/null |
    awk -v ns="$namespace" '$0 !~ ns && /typetorchW9typetorch6Tensor/ && !seen[$1]++ { bytes += strtonum("0x" $2) } END { print bytes + 0 }'
}
