...
- Mismatches become `static_assert` failures or concept violations, so the code
  fails before linking.

Wrapper methods are unconditionally inlined and forward directly to the
underlying LibTorch call. The benchmark results below show no measurable
forwarding overhead.
...
The following examples compile. See [docs/examples.md](docs/examples.md) for
the full catalog.
...
### transpose/permute track layout
...
// OK: identity permute preserves Layout::Contiguous
```
...
The following examples fail at compile time. See [docs/examples.md](docs/examples.md)
for more cases.
...
// Different dtypes or devices widen to Any rather than causing an error.
// This is intentional. Any propagates through subsequent operations.
```
...
```cpp
// Trusted internal code: no runtime checks
...
`scripts/env.local.sh` is the shell environment file sourced by `scripts/env.sh`:
...
`typetorch.local.lua` is the xmake configuration file included by `xmake.lua`:
...
In the release benchmark, forwarding overhead for the core operations was within
measurement noise. See [docs/performance-and-size.md](docs/performance-and-size.md)
for the full benchmark tables and methodology.

### Forwarding benchmark (release, 2026-06-28, 50,000 iterations)
...
In this run, every ratio is within 0.5% of 1.0.
...
- `unsafe` vs. native (21,443 vs. 21,507): -64 bytes (-0.3%). In this probe, the forwarding wrapper adds no measurable binary-size cost.
- `checked` vs. native (29,022 vs. 21,507): +7,515 bytes (+34.9%) for the wrapper and runtime boundary validation.
- `checked` vs. `unsafe` (29,022 vs. 21,443): +7,579 bytes (+35.3%) for runtime contract checks.
...
- [API notes](docs/api.md): contract details, operation style, and differences from LibTorch
